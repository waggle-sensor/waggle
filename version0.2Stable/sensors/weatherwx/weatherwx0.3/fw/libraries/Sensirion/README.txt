...
Rewritten version of the library that includes extensive changes for robustness, 
reduced code size, plus a couple of new features while maintaining backwards 
compatibility with previous versions.

    Added CRC checking (enabled by default via #define in Sensirion.h)
    Added consistent setting of the data pin internal pullup (was previously 
    undefined - depended on last bit transmitted)
