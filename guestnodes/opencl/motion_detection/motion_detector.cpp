...
int main(void)
{
    ...
    body_cascade.load("/root/opencv/data/haarcascades/haarcascade_fullbody.xml");*/
    ...
    {
        cap.read(background);
	//GaussianBlur(src, src, Size(3, 3), 1.5, 1.5, BORDER_DEFAULT);
	...
	for(uint meh=0; meh<contours.size(); meh++)
	{
	    approxPolyDP(Mat(contours[meh]), contours_poly[meh], 3, true);
	    boundRect[meh] = boundingRect(Mat(contours_poly[meh]));
	}
	for(uint blah=0; blah<contours.size(); blah++)
	{
	    if(abs(boundRect[blah].br().x - boundRect[blah].tl().x) < 100 || abs(boundRect[blah].tl().y - boundRect[blah].br().y) < 100) {continue;}
	    rectangle(src, boundRect[blah].tl(), boundRect[blah].br(), Scalar(255, 255, 0), 4, 8, 0);
	}/*
cout << ":)" << endl;
	body_cascade.detectMultiScale(src, body, 1.1, 2, 0, Size(30, 30), Size(1000, 1000));
cout << ":(" << endl;
	for(int meh=0; meh<body.size(); meh++)
	{
	    Point center(body[meh].x+body[meh].width*0.5, body[meh].y+body[meh].width*0.5);
	    ellipse(src, center, Size(body[meh].width*0.5, body[meh].height*0.5), 0, 0, 360, Scalar(255, 255, 0), 4, 8, 0);
	}*/
