...
// ************** repeated.ino
// MMA cariables to store data to average them
float previous_MMA[4];
float current_MMA[4];
float ave_MMA[4];
...
// SPV_1 variables to get data
long SPV_1_AMPV[100];
double SPV_1_AMPV_AVG = 0;

// SPV_1 variables to store data to average them
int previous_SPV1 = 0;
int current_SPV1 = 0;
int ave_SPV1 = 0;

bool flag_SPV1 = false;
bool init_SPV1 = false;

// SPV 2 variables to store data to average them
unsigned int previous_SPV2 = 0;
unsigned int current_SPV2 = 0;
unsigned int ave_SPV2 = 0;

bool flag_SPV2 = false;
bool init_SPV2 = false;
// ************** repeated.ino
...
void loop()
{
    ...
    Timer3.attachInterrupt(handler).setPeriod(1000000 * 24).start(); // print super-packet every 24 secs
    // SerialUSB.println("repeated");
    while (TIMER == false)
    {
        #ifdef AVERAGE_SENSOR_VAL
        #ifdef  AIRSENSE_INCLUDE
        airsensor_MMA8452Q();
        delay(10);
        airsense_SPV1840LRHB_1();
        delay(10);
        #endif

        #ifdef LIGHTSENSE_INCLUDE
        lightsense_SPV1840LR5HB_2();
        delay(10);
        #endif
        #endif
