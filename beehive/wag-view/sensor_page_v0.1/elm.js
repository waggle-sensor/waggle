...
Elm.Main.make = function (_elm) {
   ...
   $Task = Elm.Task.make(_elm),
   $Waggle$Update = Elm.Waggle.Update.make(_elm),
   $Waggle$View = Elm.Waggle.View.make(_elm),
   $Window = Elm.Window.make(_elm);
   var readData = Elm.Native.Task.make(_elm).performSignal("readData",
   $Waggle$Update.getData);
   var main = A3($Signal.map2,
   $Waggle$View.view,
   $Window.dimensions,
   $Waggle$Update.sensorData);
...
Elm.Waggle.Config.make = function (_elm) {
   ...
   var sensorDataUrl = "../data/current.txt";
...
Elm.Waggle.Sensor.make = function (_elm) {
   "use strict";
   _elm.Waggle = _elm.Waggle || {};
   _elm.Waggle.Sensor = _elm.Waggle.Sensor || {};
   if (_elm.Waggle.Sensor.values)
   return _elm.Waggle.Sensor.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   $moduleName = "Waggle.Sensor",
   $Dict = Elm.Dict.make(_elm),
   $QueueBuffer = Elm.QueueBuffer.make(_elm),
   $Time = Elm.Time.make(_elm);
   var InternalSensor = F2(function (a,
   b) {
      return {_: {},data: b,id: a};
   });
   var InternalValue = F4(function (a,
   b,
   c,
   d) {
      return _U.insert("physicalQuantity",
      c,
      _U.insert("units",
      b,
      _U.insert("value",a,d)));
   });
   var Sensor = F2(function (a,b) {
      return {_: {},data: b,id: a};
   });
   _elm.Waggle.Sensor.values = {_op: _op
                               ,Sensor: Sensor
...
Elm.Waggle.View.EnvSense.make = function (_elm) {
   ...
   var infraRedCamera = function (history) {
      return function () {
         var initialValue = function (seed) {
            return {ctor: "_Tuple2"
                   ,_0: 0
                   ,_1: _U.replace([["value",0]],
                   seed)};
         };
            ...
            return function () {
               ...
               {case "_Tuple2":
                  return {ctor: "_Tuple2"
                         ,_0: _v0._0 + 1
                         ,_1: _U.replace([["value"
                                          ,v1.value + _v0._1.value]],
                         v1)};}
               _U.badCase($moduleName,
               "on line 170, column 14 to 63");
            }();
         });
         ...
         var average = function (_v4) {
            return function () {
               switch (_v4.ctor)
               {case "_Tuple2":
                  return _U.replace([["value"
                                     ,_v4._1.value / $Basics.toFloat(_v4._0)]],
                    _v4._1);}
               _U.badCase($moduleName,
               "on line 161, column 44 to 81");
            }();
         };
         var casing = "TemperaturePTAT";
         var averageTemperature = $QueueBuffer.fromList($Waggle$Config.historySize)($List.map(average)(A3($Util.groupFold,
         byTimestamp,
         sumValues,
         initialValue)($List.concatMap($QueueBuffer.toList)($Dict.values(A2($Dict.remove,
         casing,
         history))))));
