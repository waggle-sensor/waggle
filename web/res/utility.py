...
def links(link, start_tag, end_tag):
	...
	pos_next = 0
	while(1):
		pos = content.find(start_tag, pos_next)
		pos_tag_end = content.find('"', pos + len(start_tag))
		pos_next = content.find(end_tag, pos_tag_end)
		if pos == -1:
			break
		link = content[pos + len(start_tag):pos_tag_end]
		#print link
		ret.append(link)
	return ret

def replace(input_file, start_tag, end_tag, original_source_code, namespace_filename, namespace_filename_without_extension):
	'''
	...
	while(1):
		pos = content.find(start_tag, pos_next)
		pos_next = content.find(end_tag, pos + len(start_tag))
		if pos == -1:
			break
		tag = content[pos + len(start_tag):pos_next]
		
		tag_end_pos = tag.find("\"")
		tag = tag[0:tag_end_pos]
		#print tag

		if original_source_code in tag:
			content = content.replace(content[pos:pos_next + len(end_tag)], A_TAG.format(namespace_filename, namespace_filename_without_extension))
...
if __name__ == '__main__':
	if len(sys.argv) < 5:
		print links(sys.argv[1], sys.argv[2], sys.argv[3])
	# elif len(sys.argv) < 5:
	# 	convert(sys.argv[1], sys.argv[2], sys.argv[3])
	elif len(sys.argv) < 8:
		replace(sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4], sys.argv[5], sys.argv[6])