...
class nc_server_class(threading.Thread, asyncore.dispatcher):
    ...
    logger = set_logging_level('nc_server_class') #logging.getLogger('nc_server_class')
    ...
    def run(self):
        try:
            ...
            self.logger.info("Listening on port_for_gn: " + str(self.port_for_gn)+"\n\n")
            while True:
                asyncore.loop(timeout=0.01, use_poll=False, map=None, count=5)                                                                         # starts the loop which constantly checks whether the socket is readable or writable
                time.sleep(0.01) 
        except Exception as inst:
            self.logger.critical("Exception in nc_server run(): " + str(inst)+"\n\n")
            time.sleep(10)
            self.run()
    ...
    def pass_thread_address(self, gn_msgs_buffer_mngr): 
        ...
        self.logger.debug("buffer address of gn_buffer_mngr saved."+"\n\n")
    
        
    ##############################################################################
    def handle_accept(self):
        try:
            self.logger.info("New GN connection available." + "\n\n")
            ...
            #self.logger.debug("Socket object corresponding to new GN created and running.")
        except:
            self.logger.critical("Error in handling the GN connection request."+ "\n\n")
            pass
    ...
    def handle_close(self):
        ...
        self.close()
        self.logger.critical("Server Socket closed."+"\n\n")
    ...
    def close_gn_socket(self):
        for socket in gn_socket_list:
            socket.handle_close()
            self.logger.info("Individual GN socket closed.")
        self.logger.critical("NC's individual GN sockets closed."+"\n\n")
